...
public class PropertiesPanel extends JPanel {
	...
	public void fillProperties() {
		...
		if(filter.getItemSpec().getCategory() == Category.BOOKS){
		...
		if(filter.getItemSpec().getCategory() == Category.ELECTRONICS){
		...
		if(filter.getItemSpec().getCategory() == Category.MUSIC) {
			add(createPropertyCheckbox( "Dołączone Video", event ->{
				filter.getItemSpec().setWithVideo(
						((JCheckBox) event.getSource()).isSelected());
				shopController.filterItems(filter);
			}));
		}
